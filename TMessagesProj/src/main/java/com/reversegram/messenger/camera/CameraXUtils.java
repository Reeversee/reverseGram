package com.reversegram.messenger.camera;
...
public class CameraXUtils {
    ...
    public static void loadSuggestedResolution() {
        ...
        getAvailableVideoSizes().values().parallelStream()
                ...
                .ifPresent(height -> {
                    ...
                    if (ReverseConfig.cameraResolution == -1 || ReverseConfig.cameraResolution > max || ReverseConfig.cameraResolution < min) {
                        ReverseConfig.editor.putInt("cameraResolution", ReverseConfig.cameraResolution = height);
                    }
                });
    }
    ...
    public static Size getPreviewBestSize() {
        ...
        return getAvailableVideoSizes().values().parallelStream()
                .filter(size -> size.getHeight() <= ReverseConfig.cameraResolution && size.getHeight() <= suggestedRes)
    ...
    public static Quality getVideoQuality() {
        return getAvailableVideoSizes().entrySet().parallelStream()
                .filter(entry -> entry.getValue().getHeight() == ReverseConfig.cameraResolution)
    ...
    private static int getSuggestedResolution(boolean isPreview) {
        ...
        switch (SharedConfig.getDevicePerformanceClass()) {
            ...
            default:
                suggestedRes = ReverseConfig.useCameraXOptimizedMode && isPreview ? 1080 : 2160;
                break;
        }
        return suggestedRes;
    }
