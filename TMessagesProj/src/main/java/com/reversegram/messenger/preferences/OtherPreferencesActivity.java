...
public class OtherPreferencesActivity extends BasePreferencesActivity {
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position == crashlyticsRow) {
            ReverseConfig.editor.putBoolean("useGoogleCrashlytics", ReverseConfig.useGoogleCrashlytics ^= true).apply();
            ((TextCell) view).setChecked(ReverseConfig.useGoogleCrashlytics);
            if (ApplicationLoader.getFirebaseCrashlytics() != null) {
                ApplicationLoader.getFirebaseCrashlytics().setCrashlyticsCollectionEnabled(ReverseConfig.useGoogleCrashlytics);
            }
        } else if (position == analyticsRow) {
            ReverseConfig.editor.putBoolean("useGoogleAnalytics", ReverseConfig.useGoogleAnalytics ^= true).apply();
            ((TextCell) view).setChecked(ReverseConfig.useGoogleAnalytics);
            if (ApplicationLoader.getFirebaseAnalytics() != null) {
                ApplicationLoader.getFirebaseAnalytics().setAnalyticsCollectionEnabled(ReverseConfig.useGoogleAnalytics);
                if (!ReverseConfig.useGoogleAnalytics) {
                    ApplicationLoader.getFirebaseAnalytics().resetAnalyticsData();
                }
            }
        } else if (position == resetSettingsRow) {
            ReverseConfig.clearPreferences();
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            switch (holder.getItemViewType()) {
                ...
                case 2:
                    ...
                    if (position == crashlyticsRow) {
                        textCell.setTextAndCheckAndIcon("Crashlytics", ReverseConfig.useGoogleCrashlytics, R.drawable.msg_report, true);
                    } else if (position == analyticsRow) {
                        textCell.setTextAndCheckAndIcon("Analytics", ReverseConfig.useGoogleAnalytics, R.drawable.msg_data, false);
                    } else if (position == deleteAccountRow) {
