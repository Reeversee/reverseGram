...
import com.reversegram.messenger.preferences.components.CameraTypeSelector;
...
public class GeneralPreferencesActivity extends BasePreferencesActivity {
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position == disableNumberRoundingRow) {
            ReverseConfig.editor.putBoolean("disableNumberRounding", ReverseConfig.disableNumberRounding ^= true).apply();
            ((TextCheckCell) view).setChecked(ReverseConfig.disableNumberRounding);
            parentLayout.rebuildAllFragmentViews(false, false);
        } else if (position == formatTimeWithSecondsRow) {
            ReverseConfig.editor.putBoolean("formatTimeWithSeconds", ReverseConfig.formatTimeWithSeconds ^= true).apply();
            ((TextCheckCell) view).setChecked(ReverseConfig.formatTimeWithSeconds);
            ...
            PopupUtils.showDialog(tabletMode, LocaleController.getString("TabletMode", R.string.TabletMode), ReverseConfig.tabletMode, getContext(), i -> {
                ReverseConfig.editor.putInt("tabletMode", ReverseConfig.tabletMode = i).apply();
                listAdapter.notifyItemChanged(tabletModeRow, payload);
                showBulletin();
            });
        } else if (position == archiveOnPullRow) {
            ReverseConfig.editor.putBoolean("archiveOnPull", ReverseConfig.archiveOnPull ^= true).apply();
            ((TextCheckCell) view).setChecked(ReverseConfig.archiveOnPull);
        } else if (position == disableUnarchiveSwipeRow) {
            ReverseConfig.editor.putBoolean("disableUnarchiveSwipe", ReverseConfig.disableUnarchiveSwipe ^= true).apply();
            ((TextCheckCell) view).setChecked(ReverseConfig.disableUnarchiveSwipe);
        } else if (position == hidePhoneNumberRow) {
            ReverseConfig.editor.putBoolean("hidePhoneNumber", ReverseConfig.hidePhoneNumber ^= true).apply();
            ((TextCheckCell) view).setChecked(ReverseConfig.hidePhoneNumber);
            parentLayout.rebuildAllFragmentViews(false, false);
            ...
            PopupUtils.showDialog(id, LocaleController.getString("ShowIdAndDc", R.string.ShowIdAndDc), ReverseConfig.showIdAndDc, getContext(), i -> {
                ReverseConfig.editor.putInt("showIdAndDc", ReverseConfig.showIdAndDc = i).apply();
                parentLayout.rebuildAllFragmentViews(false, false);
                listAdapter.notifyItemChanged(showIdAndDcRow, payload);
            });
            parentLayout.rebuildAllFragmentViews(false, false);
        } else if (position == uploadSpeedBoostRow) {
            ReverseConfig.editor.putBoolean("uploadSpeedBoost", ReverseConfig.uploadSpeedBoost ^= true).apply();
            ((TextCheckCell) view).setChecked(ReverseConfig.uploadSpeedBoost);
        } else if (position == cameraXOptimizeRow) {
            ReverseConfig.editor.putBoolean("useCameraXOptimizedMode", ReverseConfig.useCameraXOptimizedMode ^= true).apply();
            ((TextCheckCell) view).setChecked(ReverseConfig.useCameraXOptimizedMode);
            ...
            PopupUtils.showDialog(arrayList, LocaleController.getString("CameraQuality", R.string.CameraQuality), types.indexOf(ReverseConfig.cameraResolution), getContext(), i -> {
                ReverseConfig.editor.putInt("cameraResolution", ReverseConfig.cameraResolution = types.get(i)).apply();
            ...
            if (type == 17) {
                CameraTypeSelector cameraTypeSelector = new CameraTypeSelector(mContext) {
                    ...
                    protected void onSelectedCamera(int cameraSelected) {
                        ...
                        int oldValue = ReverseConfig.cameraType;
                        ReverseConfig.editor.putInt("cameraType", ReverseConfig.cameraType = cameraSelected).apply();
                        if (cameraSelected == 1) {
                            updateRowsId();
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position, boolean payload) {
            switch (holder.getItemViewType()) {
                ...
                case 5:
                    ...
                    if (position == disableNumberRoundingRow) {
                        textCheckCell.setTextAndValueAndCheck(LocaleController.getString("DisableNumberRounding", R.string.DisableNumberRounding), "1.23K -> 1,234", ReverseConfig.disableNumberRounding, true, true);
                    } else if (position == formatTimeWithSecondsRow) {
                        textCheckCell.setTextAndValueAndCheck(LocaleController.getString("FormatTimeWithSeconds", R.string.FormatTimeWithSeconds), "12:34 -> 12:34:56", ReverseConfig.formatTimeWithSeconds, true, true);
                    } else if (position == disableUnarchiveSwipeRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString("DisableUnarchiveSwipe", R.string.DisableUnarchiveSwipe), ReverseConfig.disableUnarchiveSwipe, false);
                    } else if (position == archiveOnPullRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString("ArchiveOnPull", R.string.ArchiveOnPull), ReverseConfig.archiveOnPull, true);
                    } else if (position == hidePhoneNumberRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString("HidePhoneNumber", R.string.HidePhoneNumber), ReverseConfig.hidePhoneNumber, true);
                    } else if (position == uploadSpeedBoostRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString("UploadSpeedBoost", R.string.UploadSpeedBoost), ReverseConfig.uploadSpeedBoost, false);
                    } else if (position == cameraXOptimizeRow) {
                        textCheckCell.setTextAndValueAndCheck(LocaleController.getString("PerformanceMode", R.string.PerformanceMode), LocaleController.getString("PerformanceModeInfo", R.string.PerformanceModeInfo), ReverseConfig.useCameraXOptimizedMode, true, true);
                    }
                    break;
                case 7:
                    TextSettingsCell textSettingsCell = (TextSettingsCell) holder.itemView;
                    if (position == cameraXQualityRow) {
                        textSettingsCell.setTextAndValue(LocaleController.getString("CameraQuality", R.string.CameraQuality), ReverseConfig.cameraResolution + "p", payload, false);
                    } else if (position == tabletModeRow) {
                        textSettingsCell.setTextAndValue(LocaleController.getString("TabletMode", R.string.TabletMode), tabletMode[ReverseConfig.tabletMode], payload, false);
                    } else if (position == showIdAndDcRow) {
                        textSettingsCell.setTextAndValue(LocaleController.getString("ShowIdAndDc", R.string.ShowIdAndDc), id[ReverseConfig.showIdAndDc], payload, false);
                    }
                    break;
                case 8:
                    ...
                    if (position == cameraTypeDividerRow) {
                        String advise;
                        switch (ReverseConfig.cameraType) {
                            case 0:
                ...
                case 13:
                    ...
                    if (position == downloadSpeedChooserRow) {
                        ...
                        slide.setCallback(index -> ReverseConfig.editor.putInt("downloadSpeedBoost", ReverseConfig.downloadSpeedBoost = index).apply());
                        slide.setOptions(ReverseConfig.downloadSpeedBoost, LocaleController.getString("BlurOff", R.string.BlurOff), LocaleController.getString("SpeedFast", R.string.SpeedFast), LocaleController.getString("Ultra", R.string.Ultra));
                    }
                    break;
            }
        }
